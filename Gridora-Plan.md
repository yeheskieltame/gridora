...
**Gridora is a verifiable, non-custodial adaptive-grid trading agent on BNB Chain.** It runs an adaptive maker-style grid that buys dips and sells rips inside a band, keeps your keys on your machine, signs every swap through the Trust Wallet Agent Kit, and proves what it did on-chain. Pick a coin, set a risk profile, launch — Gridora trades it autonomously inside hard guardrails and never holds custody.
...
| **Identity + journal** | — | **BridgeAgent's IdentityRegistry (ERC-8004) + TradeJournal** on BSC |
...
> Proofs use TWAK-native **ERC-8004** (identity + commit/attest metadata) because TWAK can't sign arbitrary contract calls — this keeps the whole loop self-custodial. The custom `contracts/` (IdentityRegistry/TradeJournal/StrategyLedger) are an **optional** read-only mirror, not the primary proof path.
