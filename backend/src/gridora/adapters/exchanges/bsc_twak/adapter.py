...
from .bsc_tokens import split_market
...
class BscTwakExchange:
    ...
    async def best_bid_ask(self, market: str) -> tuple[Decimal, Decimal]:
        ...
        q = await self.twak.swap("1", base, quote, chain=self.chain_key,
                                 slippage_pct=self.slippage_pct, quote_only=True)
        mid = Decimal(str(q.get("output") or q.get("price") or q.get("amountOut") or "0"))
        if mid <= 0:
    ...
    async def flatten(self, market: str) -> None:
        ...
        if free > 0:
            await self.twak.swap(str(free), base, quote, chain=self.chain_key,
                                 slippage_pct=self.slippage_pct)

    async def balance(self) -> BalanceView:
        try:
            b = await self.twak.balance(self.chain_key)
        except Exception:  # noqa: BLE001 — guards fail open on a read error
            return BalanceView()
        total_usd = Decimal(str(b.get("totalUsd", 0) or 0))
        return BalanceView(quote_free=Decimal(0), base_free=Decimal(0), equity_quote=total_usd)
        ...
        for oid, o in list(self._resting.items()):
            ...
            try:
                if o.side is Side.BUY:
                    ...
                    res = await self.twak.swap(str(spent), quote, base,
                                               chain=self.chain_key, slippage_pct=self.slippage_pct)
                    got = Decimal(str(res.get("output") or res.get("amountOut") or "0"))
                    fqty = got if got > 0 else o.qty        # real base received (>= o.qty when filled on a dip)
                    fprice = spent / fqty if fqty > 0 else o.price
                else:
                    res = await self.twak.swap(str(o.qty), base, quote,
                                               chain=self.chain_key, slippage_pct=self.slippage_pct)
                    got = Decimal(str(res.get("output") or res.get("amountOut") or "0"))
                    fqty = o.qty
