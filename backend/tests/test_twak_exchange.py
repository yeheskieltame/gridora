...
class FakeTwak:
    ...
    async def swap(self, amount, frm, to, chain=None, slippage_pct=1.0, quote_only=False):
        self.calls.append(("swap", amount, frm, to, quote_only))
        return {"output": "2.5"} if quote_only else {"hash": "0xswap"}
...
async def test_place_buy_is_limit_below_sell_is_above():
    ...
    assert ("limit", "USDT", "CAKE", "4.8", "2.4", "below") in tw.calls  # spend quote, condition below
    ...
    assert ("limit", "CAKE", "USDT", "2", "2.6", "above") in tw.calls  # sell base, condition above
...
async def test_flatten_swaps_base_to_quote():
    ...
    assert ("swap", "5", "CAKE", "USDT", False) in tw.calls
